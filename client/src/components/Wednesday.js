...
const Wednesday = ({wednesday, sideList}) => {
    ...
    const remainingProtein = subtractProtein(wednesday);
    const remainingCarbs = subtractCarbs(wednesday);
    const remainingFat = subtractFat(wednesday);
    ...
    const recipesType = wednesday.map(recipe => recipe.type);
    if(recipesType.includes('Meat')){
        Array.prototype.push.apply(wednesday, side);
    }else if(recipesType.includes('Fish')){
        Array.prototype.push.apply(wednesday, side);
    }

    const breakfast = wednesday.slice(0, 1);
    const dinner = findDinnerRecipes(wednesday);
