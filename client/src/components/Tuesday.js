...
const Tuesday = ({tuesday, sideList}) => {
    ...
    const remainingProtein = subtractProtein(tuesday);
    const remainingCarbs = subtractCarbs(tuesday);
    const remainingFat = subtractFat(tuesday);
    ...
    const recipesType = tuesday.map(recipe => recipe.type);
    if(recipesType.includes('Meat')){
        Array.prototype.push.apply(tuesday, side);
    }else if(recipesType.includes('Fish')){
        Array.prototype.push.apply(tuesday, side);
    }

    const breakfast = tuesday.slice(0, 1);
    const dinner = findDinnerRecipes(tuesday);
            ...
            <td className="macrooftheday">
                <table>
                    <tbody>
                        ...
                        <tr>
                            <td className="total-gram">{subtractProtein(tuesday)}</td>
                            <td className="total-gram">{subtractCarbs(tuesday)}</td>
                            <td className="total-gram">{subtractFat(tuesday)}</td>
                        </tr>
                    </tbody>
                </table>
            </td>
            ...
            <td className="calo">
                <table className="caloriesoftheday">
                    <tbody>
                        ...
                        <tr>
                            <td className="total-gram">{addCalories(tuesday)}</td>
                        </tr>
                    </tbody>
                </table>
            </td>
